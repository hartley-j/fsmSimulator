...
class FlipFlop:
    ...
    def update(self):
        if self.inputs["Set"] and not (self.inputs["Reset"]):
            self.output = False
        elif not (self.inputs["Set"]) and self.inputs["Reset"]:
            self.output = True
        elif not (self.inputs["Set"]) and not (self.inputs["Reset"]):
            raise Exception(f"Set and Reset are both zero for flip flop ID:{self.id}")
...
class DFlipFlop(FlipFlop):
    ...
    def set_inputs(self, *args, **kwargs):
        super(DFlipFlop, self).set_inputs(_s=kwargs["_d"], _r=not kwargs["_d"])
